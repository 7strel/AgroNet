...
class Course(models.Model):
    ...
    instructor = models.ForeignKey('agronet_auth.User', related_name='instructed_courses', on_delete=models.CASCADE, blank=True, null=True)
    students = models.ManyToManyField('agronet_auth.User', related_name='enrolled_courses', blank=True)
...
def get_upload_to_lesson(instance, filename):
    module = instance.module
    course = module.course
    return os.path.join('courses', str(course.id), str(module.id),  'lessons', filename)


class Lesson(models.Model):
    module = models.ForeignKey(Module, blank=False, null=False, related_name='lessons', on_delete=models.CASCADE)
    title = models.CharField(max_length=200, blank=False, null=False)
    content = models.FileField(blank=True, null=True, upload_to=get_upload_to_lesson)
    created_time = models.DateTimeField(auto_now_add=True)
    updated_time = models.DateTimeField(auto_now=True)
...
class Submission(models.Model):
    student = models.ForeignKey('agronet_auth.User', blank=True, null=True, related_name='submissions', on_delete=models.CASCADE)
...
class Enrollment(models.Model):
    ...
    user = models.ForeignKey('agronet_auth.User', blank=False, null=False, related_name='user_enrollments', on_delete=models.CASCADE)
