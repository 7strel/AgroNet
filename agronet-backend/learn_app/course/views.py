...
from .serializers import (
    ...
    ModuleSerializer,
    LessonSerializer, AssignmentSerializer, SubmissionSerializer, EnrollSerializer
)

from .models import (
    Category, Course, Module, Lesson, Assignment, Submission, Enrollment
)
...
def user_can_access_course(user, course):
    ...
    is_course_instructor = course.instructor.email == user.email and not user.is_student
...
class LessonDetailView(generics.RetrieveUpdateAPIView):
    ...
    def get_queryset(self):
        ...
        try:
            lession = Lesson.objects.get(pk=lession_pk)
        ...
        return Lesson.objects.filter(pk=lession_pk)
