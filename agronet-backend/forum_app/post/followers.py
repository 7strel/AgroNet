...
def follow_user(request, user_id):
    try:
        ...
        user_to_follow_profile = UserProfile.objects.get(id=user_id)
        user_to_follow = user_to_follow_profile.user
        # user_to_follow = get_object_or_404(User, id=user_to_follow_profile.user.id)
    ...
    serializer = FollowUserSerializer(data={'user_id': user_to_follow_profile.user.id})
...
def unfollow_user(request, user_id):
    ...
    # Remove the user to unfollow
    # follow from the current user's following list
