...
class CreateGetComment(APIView):
    ...
    def post(self, request, post_id):
        ...
        if serializer.is_valid():
            ...
            actor = request.user
            # verb = 'comment'
            # Notification.objects.create(user=recipient, 
            #                             content_object=post,
            #                             message=verb, 
...
class NewestAndRecentPostsView(APIView):
    """
    Returns newest posts (by created_at) and recent posts (by updated_at).
    """
    def get(self, request):
        # Newest posts
        newest_posts = Post.objects.all().order_by('-created')[:3]  # top 10 newest
        recent_posts = Post.objects.all().order_by('-updated_at')[:5]  # top 10 most recently updated

        newest_serialized = PostSerializer(newest_posts, many=True, context={'request': request}).data
        recent_serialized = PostSerializer(recent_posts, many=True, context={'request': request}).data

        base_url = request.build_absolute_uri('/')
        for post_list in [newest_serialized, recent_serialized]:
            for post in post_list:
                if "image" in post and post["image"]:
                    post["image"] = urljoin(base_url, post["image"])

        return Response({
            "newest": newest_serialized,
